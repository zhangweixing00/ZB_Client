...
namespace SharpMap
{
	...
	public class Map : IDisposable
	{
        ...
        /// <summary>
        /// 得到除了LayerName以外的所有图层的Image
        /// </summary>
        /// <returns></returns>
        public System.Drawing.Image GetMap(string LayerName)
        {
            if (Layers == null || Layers.Count == 0)
                throw new InvalidOperationException("No layers to render");
        ...
        /// <summary>
        /// 得到Map的Image，并把指定图层的Image赋给StaticLayerImage
        /// </summary>
        /// <returns></returns>
        public System.Drawing.Image GetMap(ref System.Drawing.Image StaticLayerImage, string StaticLayerName)
        {
            ...
            lock (this)
            {
                ...
                for (int i = 0; i < _Layers.Count; i++)
                {
                    ...
                    {
                        _Layers[i].Render(g, this);
                        if (_Layers[i].LayerName == StaticLayerName)
                        {
                            StaticLayerImage = (System.Drawing.Image)img.Clone();
                        }
                    }
                }
                if (MapRendered != null) MapRendered(g); //Fire render event
