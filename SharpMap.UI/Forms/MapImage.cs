	...
	public class MapImage : System.Windows.Forms.PictureBox
	{
		...
		public SharpMap.Forms.MapImage.Tools ActiveTool
		{
			...
			set
			{
                ...
                else if (value == Tools.Query)
                {
                    this.Cursor = Cursors.Help;
                }
                else if (value == Tools.BasicPoint)
                {
                    this.Cursor = Cursors.PanSouth ;
                }
                else if (value == Tools.Distance)
                {
        ...
        /// <summary>
        /// 刷新Map，并抛出包含MapLayerName和其之前的所有图层的图片的MapRefreshed事件
        /// </summary>
        public override void Refresh()
        {
            //cx 修改 ，堵塞进程
            ...
            lock (this)
            {
                ...
                if (_Map != null)
                {
                    try
                    {
                        ...
                        if (_Map.Layers == null || _Map.Layers.Count == 0)
                            ...
                            this.Image = _Map.GetMap(ref MapLayerImage, this.MapLayerName).Clone() as Image;
                        base.Refresh();
                        if (MapRefreshed != null)
                            MapRefreshed(MapLayerImage);
                    }
                    catch
                    {
                ...
                //cx 设置状态
                PublicState.Class1.LOCKFLAG = false;
            }
        ...
        /// <summary>
        /// 刷新除WithOutLayrName图层以外的所有图层。
        /// 注：不抛出MapRefreshed事件
        /// </summary>
        public void RefreshWithOutLayer(string WithOutLayrName)
        {
            ...
            lock (this)
            {
                ...
                if (_Map != null)
                {
                    ...
                    if (_Map.Layers == null || _Map.Layers.Count == 0)
                        ...
                        this.Image = _Map.GetMap(WithOutLayrName);
                    base.Refresh();
                    //if (MapRefreshed != null)
                    //    MapRefreshed(this, null);
                }
		...
		/// <param name="WorldPos"></param>
		/// <param name="ImagePos"></param>
		public delegate void RefreshEventHandler(Image MapImage);
		/// <summary>
		...
		/// </summary>
        public event RefreshEventHandler MapRefreshed;
		...
		public delegate void MapQueryHandler(SharpMap.Data.FeatureDataTable data,Point mousePoint,System.Windows.Forms.MouseButtons buttons);
		/// <summary>
        ...
        private void MapImageWheel_Thread()
        {
                            ...
                            if (_Map.Layers.Count > _queryLayerIndex && _queryLayerIndex > -1)
                            {
                                if (_Map.Layers[_queryLayerIndex].GetType() == typeof(SharpMap.Layers.VectorLayer))
                                {
                                    ...
                                    if (ds.Tables.Count > 0)
                                        if (MapQueried != null) MapQueried(ds.Tables[0], e.Location,e.Button);
                                        else
                                            if (MapQueried != null) MapQueried(new SharpMap.Data.FeatureDataTable(), e.Location,e.Button);
                                }
                            }
                            else
